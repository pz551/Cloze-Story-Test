    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            if preds is None:
                ...
                input_id_rec.append(inputs['input_ids'])
            else:
        ...
        with open("prediction.csv", mode="w", newline='') as f:
            ...
            #prediction_writer.writerow([len(input_id_rec), len(preds)])
            
            # for i in range(len(input_id_rec)):
            #     prediction_writer.writerow([input_id_rec[i]])
            for i in range(len(preds)):
                prediction_writer.writerow([preds[i]+1])
            '''
            for i in range(len(preds)):
                prediction_writer.writerow([input_id_rec[i], preds[i] + 1])
            '''
